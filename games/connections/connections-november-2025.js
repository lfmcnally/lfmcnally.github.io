// Classicalia Connections - November 2025
// Daily Latin & Classics word puzzles

const connectionsNovember2025 = [
    {
        day: 14,
        month: "November",
        year: 2025,
        categories: [
            {
                name: "TYPES OF SALAD",
                words: ["Caesar", "Waldorf", "Cobb", "Nicoise"],
                difficulty: 4,
                color: "#8b5cf6" // purple - fiendish
            },
            {
                name: "ROMAN EMPERORS",
                words: ["Trajan", "Hadrian", "Claudius", "Nero"],
                difficulty: 2,
                color: "#eab308" // yellow - tricky (Caesar is missing!)
            },
            {
                name: "1ST DECLENSION NOUNS",
                words: ["puella", "mensa", "via", "porta"],
                difficulty: 1,
                color: "#22c55e" // green - straightforward
            },
            {
                name: "LATIN WORDS FOR 'WATER'",
                words: ["aqua", "unda", "flumen", "fons"],
                difficulty: 3,
                color: "#3b82f6" // blue - devious
            }
        ]
    },
    {
        day: 15,
        month: "November",
        year: 2025,
        categories: [
            {
                name: "ROMAN BUILDINGS/STRUCTURES",
                words: ["ARCH", "CIRCUS", "BATHS", "FORUM"],
                difficulty: 1,
                color: "#22c55e" // green - straightforward
            },
            {
                name: "PLANETS (NAMED AFTER ROMAN GODS)",
                words: ["MARS", "MERCURY", "PLUTO", "JUNO"],
                difficulty: 2,
                color: "#eab308" // yellow - tricky
            },
            {
                name: "FLOWERS NAMED AFTER MYTHOLOGICAL FIGURES",
                words: ["IRIS", "NARCISSUS", "AJAX", "ECHO"],
                difficulty: 3,
                color: "#3b82f6" // blue - devious
            },
            {
                name: "FIRST NAMES OF ROMAN EMPERORS",
                words: ["PHILIP", "TITUS", "MARK", "LUCIUS"],
                difficulty: 4,
                color: "#8b5cf6" // purple - fiendish (Philip the Arab, Titus, Marcus Aurelius, Lucius Verus)
            }
        ]
    },
     {
        day: 16,
        month: "November",
        year: 2025,
        categories: [
            {
                name: "COMPANIES/BRANDS",
                words: ["NIKE", "AMAZON", "HERMES", "MARS"],
                difficulty: 1,
                color: "#22c55e" // green - straightforward
            },
            {
                name: "CHILDREN OF ZEUS",
                words: ["HERACLES", "DIONYSUS", "PERSEPHONE", "MINOS"],
                difficulty: 2,
                color: "#eab308" // yellow - tricky
            },
            {
                name: "___ [HERACLES' LABOURS]",
                words: ["BOAR", "BIRDS", "HYDRA", "LION"],
                difficulty: 3,
                color: "#3b82f6" // blue - devious
            },
            {
                name: "HALF-HUMAN HYBRIDS",
                words: ["CENTAUR", "MEDUSA", "PAN", "HARPY"],
                difficulty: 4,
                color: "#8b5cf6" // purple - fiendish
            }
        ]
    },
    {
        day: 17,
        month: "November",
        year: 2025,
        categories: [
            {
                name: "___ HORSE",
                words: ["TROJAN", "CLOTHES", "RACE", "GIFT"],
                difficulty: 1,
                color: "#22c55e" // green - straightforward
            },
            {
                name: "CHARACTERS IN THE ILIAD",
                words: ["ACHILLES", "HECTOR", "DIOMEDES", "PARIS"],
                difficulty: 2,
                color: "#eab308" // yellow - tricky
            },
            {
                name: "CLEANING PRODUCTS",
                words: ["AJAX", "FLASH", "DAWN", "FAIRY"],
                difficulty: 3,
                color: "#3b82f6" // blue - devious
            },
            {
                name: "WORDS CONTAINING 'AE'",
                words: ["AENEAS", "CAESAR", "DAEDALUS", "PHAEDRA"],
                difficulty: 4,
                color: "#8b5cf6" // purple - fiendish
            }
        ]
    },
   {
        day: 18,
        month: "November",
        year: 2025,
        categories: [
            {
                name: "PARTS OF A GREEK TEMPLE",
                words: ["COLUMN", "PEDIMENT", "CELLA", "STYLOBATE"],
                difficulty: 1,
                color: "#22c55e" // green - straightforward
            },
            {
                name: "ANCIENT LANGUAGES",
                words: ["ARAMAIC", "SANSKRIT", "COPTIC", "PHOENICIAN"],
                difficulty: 2,
                color: "#eab308" // yellow - tricky
            },
            {
                name: "WORDS BEGINNING WITH DAYS OF THE WEEK",
                words: ["MONSTER", "THUNDER", "FRIEZE", "SATURN"],
                difficulty: 3,
                color: "#3b82f6" // blue - devious
            },
            {
                name: "WORDS ENDING IN LATIN PREPOSITIONS",
                words: ["VORTEX", "NOMAD", "LATIN", "TRADE"],
                difficulty: 4,
                color: "#8b5cf6" // purple - fiendish
            }
        ]
    },
    {
        day: 19,
        month: "November",
        year: 2025,
        categories: [
            {
                name: "LATIN WORDS FOR 'GOOD'",
                words: ["bonus", "melior", "optimus", "probus"],
                difficulty: 1,
                color: "#22c55e"
            },
            {
                name: "THINGS IN A ROMAN HOUSE",
                words: ["atrium", "peristylium", "tablinum", "triclinium"],
                difficulty: 3,
                color: "#3b82f6"
            },
            {
                name: "WORDS ENDING IN -TOR",
                words: ["victor", "narrator", "senator", "gladiator"],
                difficulty: 2,
                color: "#eab308"
            },
            {
                name: "FALSE FRIENDS (LATIN ≠ ENGLISH)",
                words: ["camera", "silva", "arena", "campus"],
                difficulty: 4,
                color: "#8b5cf6" // room/forest/sand/field not what they seem!
            }
        ]
    },
    {
        day: 20,
        month: "November",
        year: 2025,
        categories: [
            {
                name: "RIVERS",
                words: ["Tiber", "Thames", "Rhine", "Danube"],
                difficulty: 2,
                color: "#eab308"
            },
            {
                name: "FAMOUS ROMAN BATTLES",
                words: ["Cannae", "Actium", "Pharsalus", "Zama"],
                difficulty: 3,
                color: "#3b82f6"
            },
            {
                name: "LATIN 3RD CONJUGATION VERBS",
                words: ["duco", "mitto", "pono", "scribo"],
                difficulty: 1,
                color: "#22c55e"
            },
            {
                name: "THINGS HANNIBAL USED",
                words: ["elephas", "navis", "exercitus", "sagittae"],
                difficulty: 4,
                color: "#8b5cf6"
            }
        ]
    },
    {
        day: 21,
        month: "November",
        year: 2025,
        categories: [
            {
                name: "GREEK PHILOSOPHERS",
                words: ["Socrates", "Plato", "Aristotle", "Epicurus"],
                difficulty: 1,
                color: "#22c55e"
            },
            {
                name: "ROMAN PHILOSOPHERS/AUTHORS",
                words: ["Cicero", "Seneca", "Marcus", "Lucretius"],
                difficulty: 3,
                color: "#3b82f6" // Marcus = Marcus Aurelius
            },
            {
                name: "LATIN WORDS FOR 'VOICE/WORD'",
                words: ["vox", "verbum", "nomen", "oratio"],
                difficulty: 2,
                color: "#eab308"
            },
            {
                name: "THINGS BEGINNING WITH 'PH'",
                words: ["philosophy", "pharmacy", "phoenix", "phantom"],
                difficulty: 4,
                color: "#8b5cf6" // From Greek, not obvious connection
            }
        ]
    },
    {
        day: 22,
        month: "November",
        year: 2025,
        categories: [
            {
                name: "LATIN WORDS FOR 'SMALL'",
                words: ["parvus", "minor", "pusillus", "exiguus"],
                difficulty: 1,
                color: "#22c55e"
            },
            {
                name: "OLYMPIAN GODS (GREEK NAMES)",
                words: ["Zeus", "Hera", "Poseidon", "Athena"],
                difficulty: 2,
                color: "#eab308"
            },
            {
                name: "PIZZA TYPES",
                words: ["Margherita", "Romana", "Napoletana", "Siciliana"],
                difficulty: 4,
                color: "#8b5cf6" // Italian places but it's pizza!
            },
            {
                name: "LATIN WORDS FOR 'WAR'",
                words: ["bellum", "pugna", "proelium", "certamen"],
                difficulty: 3,
                color: "#3b82f6"
            }
        ]
    },
    {
        day: 23,
        month: "November",
        year: 2025,
        categories: [
            {
                name: "TOOLS OF WRITING",
                words: ["stilus", "cera", "papyrus", "membrana"],
                difficulty: 2,
                color: "#eab308"
            },
            {
                name: "LATIN PERFECT TENSE ENDINGS",
                words: ["avi", "ivi", "ui", "si"],
                difficulty: 1,
                color: "#22c55e"
            },
            {
                name: "CHARACTERS FROM AENEID",
                words: ["Aeneas", "Dido", "Turnus", "Anchises"],
                difficulty: 3,
                color: "#3b82f6"
            },
            {
                name: "OPERAS BY VERDI/PURCELL",
                words: ["Aida", "Dido", "Rigoletto", "Nabucco"],
                difficulty: 4,
                color: "#8b5cf6" // Dido appears twice!
            }
        ]
    },
    {
        day: 24,
        month: "November",
        year: 2025,
        categories: [
            {
                name: "ROMAN CLOTHING",
                words: ["toga", "tunica", "stola", "palla"],
                difficulty: 1,
                color: "#22c55e"
            },
            {
                name: "WORDS MEANING 'ROAD/WAY'",
                words: ["via", "iter", "cursus", "semita"],
                difficulty: 2,
                color: "#eab308"
            },
            {
                name: "FAMOUS ROMAN ROADS",
                words: ["Appia", "Flaminia", "Aurelia", "Salaria"],
                difficulty: 3,
                color: "#3b82f6"
            },
            {
                name: "CONSTELLATIONS",
                words: ["Orion", "Gemini", "Virgo", "Aquarius"],
                difficulty: 4,
                color: "#8b5cf6" // Virgo could be virgin/maiden in Latin
            }
        ]
    },
    {
        day: 25,
        month: "November",
        year: 2025,
        categories: [
            {
                name: "LATIN WORDS FOR 'LOVE'",
                words: ["amo", "diligo", "cupio", "desidero"],
                difficulty: 1,
                color: "#22c55e"
            },
            {
                name: "MYTHICAL CREATURES",
                words: ["Medusa", "Minotaurus", "Cyclops", "Centaurus"],
                difficulty: 2,
                color: "#eab308"
            },
            {
                name: "THINGS WITH ONE EYE",
                words: ["Cyclops", "Polyphemus", "monocle", "webcam"],
                difficulty: 4,
                color: "#8b5cf6" // Cyclops appears twice! Polyphemus is a Cyclops
            },
            {
                name: "LATIN WORDS FOR 'SEE'",
                words: ["video", "specto", "aspicio", "intueor"],
                difficulty: 3,
                color: "#3b82f6"
            }
        ]
    },
    {
        day: 26,
        month: "November",
        year: 2025,
        categories: [
            {
                name: "LATIN WORDS FOR 'MAKE/DO'",
                words: ["facio", "ago", "gero", "perficio"],
                difficulty: 1,
                color: "#22c55e"
            },
            {
                name: "THE TWELVE LABOURS",
                words: ["Leo", "Hydra", "Cerberus", "Augeas"],
                difficulty: 3,
                color: "#3b82f6"
            },
            {
                name: "ZODIAC SIGNS",
                words: ["Leo", "Gemini", "Virgo", "Sagittarius"],
                difficulty: 4,
                color: "#8b5cf6" // Leo appears twice!
            },
            {
                name: "LATIN 4TH CONJUGATION VERBS",
                words: ["audio", "venio", "dormio", "sentio"],
                difficulty: 2,
                color: "#eab308"
            }
        ]
    },
    {
        day: 27,
        month: "November",
        year: 2025,
        categories: [
            {
                name: "PARTS OF A ROMAN VILLA",
                words: ["villa", "hortus", "culina", "cubiculum"],
                difficulty: 2,
                color: "#eab308"
            },
            {
                name: "LATIN WORDS FOR 'SPEAK'",
                words: ["dico", "loquor", "narro", "clamo"],
                difficulty: 1,
                color: "#22c55e"
            },
            {
                name: "ROMAN FESTIVALS",
                words: ["Saturnalia", "Lupercalia", "Vestalia", "Floralia"],
                difficulty: 3,
                color: "#3b82f6"
            },
            {
                name: "THINGS ENDING IN -ALIA",
                words: ["regalia", "memorabilia", "Australia", "Italia"],
                difficulty: 4,
                color: "#8b5cf6" // Tricky overlap with festivals!
            }
        ]
    },
    {
        day: 28,
        month: "November",
        year: 2025,
        categories: [
            {
                name: "LATIN PRONOUNS",
                words: ["ego", "tu", "nos", "vos"],
                difficulty: 1,
                color: "#22c55e"
            },
            {
                name: "PARTS OF THE BODY",
                words: ["caput", "manus", "pes", "corpus"],
                difficulty: 2,
                color: "#eab308"
            },
            {
                name: "THE SEVEN HILLS OF ROME",
                words: ["Palatine", "Capitoline", "Aventine", "Quirinal"],
                difficulty: 3,
                color: "#3b82f6"
            },
            {
                name: "WORDS IN 'E PLURIBUS UNUM'",
                words: ["e", "pluribus", "unum", "ex"],
                difficulty: 4,
                color: "#8b5cf6" // e = ex, tricky!
            }
        ]
    },
    {
        day: 29,
        month: "November",
        year: 2025,
        categories: [
            {
                name: "GREEK EPIC POETS",
                words: ["Homer", "Hesiod", "Apollonius", "Callimachus"],
                difficulty: 2,
                color: "#eab308"
            },
            {
                name: "LATIN WORDS FOR 'BOOK'",
                words: ["liber", "codex", "volumen", "pagina"],
                difficulty: 1,
                color: "#22c55e"
            },
            {
                name: "SIMPSONS CHARACTERS",
                words: ["Homer", "Marge", "Bart", "Lisa"],
                difficulty: 4,
                color: "#8b5cf6" // Homer appears twice!
            },
            {
                name: "TYPES OF GOVERNMENT",
                words: ["monarchia", "democratia", "oligarchia", "tyrannis"],
                difficulty: 3,
                color: "#3b82f6"
            }
        ]
    },
    {
        day: 30,
        month: "November",
        year: 2025,
        categories: [
            {
                name: "LATIN WORDS FOR 'TIME'",
                words: ["tempus", "hora", "dies", "annus"],
                difficulty: 1,
                color: "#22c55e"
            },
            {
                name: "ROMAN EMPERORS (FLAVIAN)",
                words: ["Vespasian", "Titus", "Domitian", "Nerva"],
                difficulty: 3,
                color: "#3b82f6"
            },
            {
                name: "SHAKESPEAREAN PLAYS",
                words: ["Julius", "Antony", "Coriolanus", "Titus"],
                difficulty: 4,
                color: "#8b5cf6" // Titus appears twice! Julius = Julius Caesar
            },
            {
                name: "LATIN QUESTION WORDS",
                words: ["quis", "quid", "cur", "ubi"],
                difficulty: 2,
                color: "#eab308"
            }
        ]
    }
];
